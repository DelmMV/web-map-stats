...
const TopUsers = ({ userId, admins }) => {
	...
	return (
		<Box p={5} maxW='800px' mx='auto' marginBottom='30px'>
			<Box align='center'>
				<Badge mb={5} fontSize={27} bgColor='gray.100' borderRadius={10}>
					Топ 50
				</Badge>
			</Box>
			<Box display='flex' flexDirection='row' alignItems='center' m={0} p={0}>
				<Select value={period} onChange={e => setPeriod(e.target.value)} mb={5}>
			...
			{loading ? (
				...
				<Box>
					...
					<Tabs>
						...
						<TabPanels>
							<TabPanel>{renderUsersList(topUsers)}</TabPanel>
							<TabPanel>{renderSessionsList(topSessions)}</TabPanel>
							<TabPanel>{renderDailyDistancesList(topDailyDistances)}</TabPanel>
						</TabPanels>
					</Tabs>
				</Box>
			)}
